...
const int ledG = 12;
const int ledR = 14;
const int ledB = isHost ? LED_BUILTIN : 15;
...
void setBlinkerPeriod() {
  if (state == noConnectionState) {
    blinker.setPeriod(350, 350);
  } else if (!isCamAvailable) {
    blinker.setPeriod(50, 100);
  } else {
    blinker.setPeriod(3000, 500);
  }
}
