...
int dataConstrainX = 1024;
int dataConstrainY = 768;
...
void draw() {
  ...
  drawDataPoint(dataConstrainX - p1x, p1y, p1color);  
  drawDataPoint(dataConstrainX - p2x, p2y, p2color);
  drawDataPoint(dataConstrainX - p3x, p3y, p3color);  
  drawDataPoint(dataConstrainX - p4x, p4y, p4color); 
  drawDataPoint(finalX, dataConstrainY - finalY, finalColor);
...
void drawDataPoint(int px, int py, color c) {
  ...
  fill(c);  
  if (px != 1023 && px != 0) {
    ellipse(map(px, 0, dataConstrainX, 0, screenX), map(py, 0, dataConstrainY, 0, screenY), 20, 20);
  }
}
