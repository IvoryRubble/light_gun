...
void getPosition() {    // Get tilt adjusted position from IR postioning camera
  ...
  if (myDFRobotIRPosition.available()) {
    for (int i = 0; i < 4; i++) {
      ...
      positionX[i] = myDFRobotIRPosition.readY(i);
      positionY[i] = myDFRobotIRPosition.readX(i);
    }
    ...
    if (positionX[0] != 1023 && positionX[1] != 1023) {
      ...
      finalX = 384 + cos(atan2(twoY - oneY, twoX - oneX) * -1) * (((oneX - twoX) / 2 + twoX) - 384) - sin(atan2(twoY - oneY, twoX - oneX) * -1) * (((oneY - twoY) / 2 + twoY) - 512);
      finalY = 512 + sin(atan2(twoY - oneY, twoX - oneX) * -1) * (((oneX - twoX) / 2 + twoX) - 384) + cos(atan2(twoY - oneY, twoX - oneX) * -1) * (((oneY - twoY) / 2 + twoY) - 512);
    }
  } 
