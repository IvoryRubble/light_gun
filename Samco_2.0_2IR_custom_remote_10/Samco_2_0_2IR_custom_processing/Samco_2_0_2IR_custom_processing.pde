...
float finalPointConstrainX = 4;
float finalPointConstrainY = 4;
...
void draw() {
  ...
  drawFinalPoint(finalX, finalY, finalColor);
  ...
  text("finalX: " + finalX + "\t finalY: " + finalY, 10, 10 + 25 + 25 + 25 + 25 + 25);
}
...
void drawDataPointReversed(float px, float py, color c) {
  if (px != 1023 && px != 0) {
    drawDataPoint(dataConstrainX - px, dataConstrainY - py, c);
  }
}

void drawDataPoint(float px, float py, color c) {
  ellipseMode(RADIUS);
  fill(c);  
  ellipse(map(px, 0, dataConstrainX, borderX, screenX - borderX), map(py, 0, dataConstrainY, borderY, screenY - borderY), 20, 20);
}

void drawFinalPoint(float finalX, float finalY, color c) {
  ellipseMode(RADIUS);
  fill(c);  
  ellipse(map(finalX, -finalPointConstrainX, finalPointConstrainX, borderX, screenX - borderX), map(finalY, -finalPointConstrainY, finalPointConstrainX, borderY, screenY - borderY), 20, 20);
}
...
void serialEvent (Serial port) {
  ...
  if (inString != null) {
    ...
    if (inputS.length >= 13) {
      ...
      finalX = float(inputS[8]);
      finalY = float(inputS[9]);
